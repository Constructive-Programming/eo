...
class SchemesBench extends JmhDefaults:
  ...
  val eoCrossFusedG = Schemes.ana[BinF, Int, Bin](eoTypedCoalg).cross(Schemes.cata(eoTypedSum))

  @Benchmark def eoCrossFused: Int = eoCrossFusedG.get(Depth)
  @Benchmark def eoCrossMaterialized: Int = eoCataG.get(eoAnaR.reverseGet(Depth))
