...
/** JsonTraversal vs. naive decode-modify-encode across an array.
  *
  * The story: uppercasing every `name` across an N-element array of records nested inside an outer
  * object. The cursor-backed traversal touches only:
  *   1. the outer object once (prefix walk),
  *   2. each array element's `name` field,
  *   3. a single array rebuild.
  *
  * The naive comparator decodes the entire `Basket` (array and all), maps `copy(name = ...)` over
  * the whole array, re-encodes.
  *
