...
/** Shared nested-case-class fixture for per-optic benchmarks.
  *
  * Seven levels (`Nested0` … `Nested6`) so each bench can measure
  * how composed optics scale with depth — `_0` for the single-hop
  * cost, `_3` for a reasonable production depth, `_6` for the
  * stress case.
  *
  * Each leaf (`Nested0`) carries three kinds of focus:
  *   - `value: Int` — drives Lens / Getter / Setter.
  *   - `flag: Option[String]` — drives Optional (the conditional
  *     branch is exactly the `None` side of the `Option`).
  *   - `items: List[Int]` — drives Fold over a `Foldable` carrier.
  *
  * `Nested1` … `Nested6` are a linear chain with a single `n`
  * field each, so depth-to-depth lens composition is
  * unambiguous.
  */
...
object Nested:
  ...
  /** A populated leaf with a non-empty `flag` and a 16-element
    * list — so Optional's Some-branch and Fold's list-walk both
    * do real work. */
  ...
  /** A leaf with an empty `flag`. Some Optional benches run both
    * variants to show the Some / None cost split. */
