...
import eo.optics.{Setter => EoSetter}
import eo.optics.Optic.*

import monocle.{Setter => MSetter}

/** `Setter.modify` at the leaf plus deep manual composition,
  * paired EO vs Monocle.
  *
  * **Scope note.** EO's `Setter` carrier is `SetterF`, which
  * has a `ForgetfulFunctor[SetterF]` instance but no
  * `AssociativeFunctor[SetterF, X, Y]` instance — so two Setters
  * cannot be composed via `Optic.andThen`. For depth-3 / depth-6
  * writes the bench nests `modify` calls — each outer Setter
  * modifies the next `n`, with the innermost modifying the
  * leaf's `value`. Monocle's first-class `Setter.andThen`
  * produces the equivalent composed Setter on its side.
  */
...
class SetterBench:
  ...
  private val mN1: MSetter[Nested1, Nested0] = MSetter[Nested1, Nested0](f => x => x.copy(n = f(x.n)))
  private val mN2: MSetter[Nested2, Nested1] = MSetter[Nested2, Nested1](f => x => x.copy(n = f(x.n)))
  private val mN3: MSetter[Nested3, Nested2] = MSetter[Nested3, Nested2](f => x => x.copy(n = f(x.n)))
  private val mN4: MSetter[Nested4, Nested3] = MSetter[Nested4, Nested3](f => x => x.copy(n = f(x.n)))
  private val mN5: MSetter[Nested5, Nested4] = MSetter[Nested5, Nested4](f => x => x.copy(n = f(x.n)))
  private val mN6: MSetter[Nested6, Nested5] = MSetter[Nested6, Nested5](f => x => x.copy(n = f(x.n)))
