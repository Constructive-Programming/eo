...
| `avro` | `avro/` | `cats-eo-avro` | Apache Avro optic integration; the `eo.avro.circe` sub-package is the structural Avro ↔ circe bridge (`AvroJson`), `eo.avro.jsoniter` the AST-free Avro ↔ JSON-bytes bridge (`AvroJsoniter` + the `.json` / `.avro` cursor faces), and `eo.avro.vulcan` bridges `vulcan.Codec` → `AvroCodec` (`AvroVulcan`) — circe, jsoniter-scala-core, cats-eo-jsoniter, and vulcan are `Optional` deps, callers add them themselves. NB avro depends on the jsoniter MODULE (Optional); the reverse would be a project cycle, so the cross-format bridge specs live in `avro/src/test` |
