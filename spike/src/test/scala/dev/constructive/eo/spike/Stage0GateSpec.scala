...
/** Stage-0 gate correctness: the hand-written, droste-interop, and eo encoding-A
  * unfolds must produce the SAME `Json` for the same seed. (The ceremony comparison
  * that actually drives the go/no-go decision is recorded in the verdict doc, not
  * asserted here — this spec only proves the three are equivalent so the comparison
  * is apples-to-apples.)
  */
