...
/** Proves eo's closure encoding handles the **effectful** fused unfold→fold: the eo version agrees
  * with both droste's `hyloM` and the hand-written recursion, on both the success path (Fibonacci
  * sums) and the failure path (the blocked seed short-circuits to `Left` identically). Refutes the
  * earlier mistaken "eo is pure-only" claim.
  */
