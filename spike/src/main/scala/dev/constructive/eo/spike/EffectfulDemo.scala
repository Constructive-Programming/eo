...
/** arbo-shaped demonstrator: an **effectful, fused, fail-able** unfold→fold — the real
  * recursion-scheme shape (`~/workspace/crypto/arbo` `Calculator.selection` via `elgotM`), not the
  * pure toy. Computes a Fibonacci-style sum over a tree whose expansion is an effectful, fail-able
  * "options" lookup (`M = Either[String, *]`): seed 13 is "blocked" (`Left`), so any seed whose
  * expansion reaches 13 fails — exactly like arbo pruning / a failing I/O call. Built THREE ways so
  * the comparison is real.
  *
  * Purpose: refute the earlier (mistaken) claim that eo's closure encoding is "pure only". eo
  * threads the effect exactly as `Plated.rewrite` threads `Eval` — via a monadic traverse over the
  * children. The effectful case is no obstacle.
  */
object EffectfulDemo:

  type M[A] = Either[String, A]

  /** The shared effectful, fail-able expansion (the "I/O" all three implementations use). Leaf when
    * no options; failure at the blocked seed; otherwise the two child seeds.
    */
