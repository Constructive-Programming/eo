...
/** Compatibility alias for the multi-field Prism shape.
  ...
  * surface that mirrored [[JsonPrism]] line-for-line. The two classes only differed in the
  * "where does the focus live" axis (single leaf vs assembled NamedTuple from selected fields),
  * which is now factored into [[JsonFocus]]. A multi-field prism is just a `JsonPrism[NT]` whose
  * focus happens to be a [[JsonFocus.Fields]]; this alias keeps old type ascriptions compiling.
  *
