...
/** A specialised Prism from [[io.circe.Json]] to a native type `A`, backed by a circe `Encoder[A]`
  * / `Decoder[A]` pair and a flat `Array[String]` path that tracks the cursor steps.
  *
  ...
  *
  * Storage is a flat `Array[String]` of field names (the path from the root Json to the focused
  * value). Each `.field(_.name)` call copies the array and appends one step. Walking the path at
  * `modify` / `transform` time is an imperative loop that pulls JsonObjects directly via
  * `Json.asObject` / `JsonObject.apply` — skipping `HCursor` entirely — and rebuilds by walking up
  * through `JsonObject.add`, which keeps the underlying `LinkedHashMap` representation and avoids
  * the expensive `LinkedHashMap → MapAndVector` conversion that `HCursor.set.top` triggers (≈27 %
  * of runtime on the narrow d2 bench before this optimisation).
  *
  * Forgiving semantics: `modify` returns the input Json unchanged if any step on the path is
  * missing, if the Json at that step isn't a JsonObject, or if the focused value doesn't decode.
  * `transform` applies the user's function to `Json.Null` when the leaf is absent and inserts the
  * result at the missing field.
  *
  * Failure diagnostics: the abstract `to` method (used through the generic Optic extensions)
  * returns `Left((DecodingFailure, HCursor))` so callers can both diagnose
  * (`DecodingFailure.history`) and recover raw Json (`HCursor.value` / `HCursor.top`).
  */
