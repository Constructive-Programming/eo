...
import eo.optics.{AffineFold, Lens, Optic}
...
/** Unit 7: cross-carrier composition regression specs.
  ...
  * `eo.generics`, and `AffineFold`. Chains (4) and (5) use the MANUAL composition idiom because
  * `JsonTraversal` is deliberately outside the Optic trait (see `site/docs/concepts.md` — "would
  * have to invent an artificial `to`"). Future Considerations in
...
class CrossCarrierCompositionSpec extends Specification:
  ...
  "(1) plain Lens → JsonFieldsPrism → AffineFold, composed read-only chain" should {
    ...
    val boxToFields: Optic[Box, Box, NameAge, NameAge, eo.data.Affine] =
  ...
  "(3) generics lens[S](_.field) → multi-field JsonPrism (.fields), native .andThen" should {
    ...
    // Here we pull the outer lens in through eo.generics to witness
    ...
    // extension on Affine).
    val chain: Optic[Envelope, Envelope, NameAge, NameAge, eo.data.Affine] =
...
object CrossCarrierCompositionSpec:
  ...
  // Envelope has a tag alongside payload so the `eo.generics.lens`
