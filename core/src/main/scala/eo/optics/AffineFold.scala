...
/** Constructors for `AffineFold` — the read-only zero-or-one-focus optic, backed by the `Affine`
  ...
  *
  * An `AffineFold[S, A]` (short for `Optic[S, Unit, A, A, Affine]`) is the read-only counterpart
  * to [[Optional]]: it exposes the focus when present via `.getOption(s)` and `.foldMap(f)(s)`,
  * but has no `.modify` / `.replace` because `T = Unit` provides no way back to `S`.
  *
...
object AffineFold:
  ...
  /** Construct an AffineFold from a `matches: S => Option[A]` partial-read function.
    *
    * `.getOption(s)` returns the result of `matches` directly; `.foldMap(f)(s)` yields
    * `f(a)` when `matches(s)` is `Some(a)` and `Monoid[M].empty` when it's `None`.
    *
    * @group Constructors
    *
    * @example
...
/** Concrete `Optic` subclass for an `AffineFold`. Stores the `matches: S => Option[A]` directly
  * so the hot path fuses `.getOption` without going through the generic carrier round-trip.
  *
  * Extends `Optic[S, Unit, A, A, Affine]` — satisfies the generic Optic contract so `.foldMap`
  * and `.modifyA` reach through the `ForgetfulFold[Affine]` / `ForgetfulTraverse[Affine,
  * Applicative]` instances — while also exposing a direct `.getOption` that skips the Affine
