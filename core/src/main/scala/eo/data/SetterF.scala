...
/** Carrier for the `Setter` family — pairs a source `Fst[A]` with a
  * continuation `Snd[A] => B` that decides what to do with the focus
  * when it's written back.
  *
  * Like [[Affine]], SetterF's `A` is encoded as a `Tuple2` that
  * threads both halves of the write path together. SetterF has no
  * `AssociativeFunctor` instance: composing two `SetterF` optics
  * via `Optic.andThen` is not yet supported. Compose a Lens chain
...
object SetterF:
  ...
  /** `ForgetfulFunctor[SetterF]` — maps the right-side continuation
    * through `f`, leaving the source unchanged. Unlocks `.modify`
    * and `.replace` on Setter-carrier optics.
    *
    * @group Instances */
  ...
  /** `ForgetfulTraverse[SetterF, Distributive]` — lifts an
    * effectful `B => G[C]` through the continuation using
    * `Distributive[G]` (a stronger counterpart to `Applicative` that
    * suits the read-once / write-once Setter shape).
    *
    * @group Instances */
