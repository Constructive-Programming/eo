...
/** Carrier for the `PowerSeries`-style `Traversal`: pairs an existential leftover `Snd[A]` with a
  * length-indexed [[Vect]] of focused elements `B`.
  *
  ...
  * `AssociativeFunctor`, and `Forget[F]` (the `each` carrier) does not. `PowerSeries` provides that
  * instance — see [[PowerSeries.assoc]] — at a super-linear runtime cost (indexing, per-element
  * vector concatenation).
  *
  * Rule of thumb: use `Traversal.each[F, A, B]` for plain element-wise modify; reach for
  * `Traversal.powerEach[F, A]` only when the chain continues past the traversal with another optic.
  *
  * See `benchmarks/src/main/scala/eo/bench/PowerSeriesBench.scala` for the runtime cost breakdown.
  */
  ...
  given tuple2ps: Composer[Tuple2, PowerSeries] with
    ...
    def to[S, T, A, B](o: Optic[S, T, A, B, Tuple2]): Optic[S, T, A, B, PowerSeries] =
      new Optic[S, T, A, B, PowerSeries]:
        type X = (1, o.X)
        def to: S => PowerSeries[X, A] = s => PowerSeries(o.to(s).fmap(Vect.of))
