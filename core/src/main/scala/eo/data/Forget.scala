...
/** Adapt a `F[_]` container to the two-parameter carrier shape by wrapping it under the phantom
  ...
  *
  * Used by [[eo.optics.Fold]], [[eo.optics.Traversal.forEach]], and the algebraic-lens family
  * ([[eo.optics.AlgebraicLens]]) as a uniform "F-shape carrier" whose optic-level capabilities
  * scale with the typeclasses `F` itself admits.
  */
