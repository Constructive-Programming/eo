...
/** Length-indexed immutable vector: `Vect[N, A]` holds exactly `N` elements of type `A`, with the
  * length visible at the type level.
  *
  ...
  *
  * `slice` / `++` / `+:` / `:+` all operate at the type level on the length index, so invalid
  * accesses (`slice(5, 3)` on a `Vect[2, A]`) are rejected at compile time. The phantom `N <: Int`
  * is the whole point — runtime bounds checks are not a substitute.
  *
  * Primary consumer is [[PowerSeries]] (per-index composition of traversals); [[FixedTraversal]]
  * and the `Traversal.two` / `.three` / `.four` constructors also build on it.
  *
  * @tparam N
  *   length of the vector (0, 1, 2, …)
  * @tparam A
  *   element type
  */
  ...
  def slice[L <: Int](offset: Int, len: L): Vect[L, A] = this match
    ...
    case AdjacentVect(init, tail)        =>
      if (offset + len <= init.size) init.slice(offset, len)
        ...
        (init.slice(offset, init.size - offset) ++ tail.slice(
          0,
          len + offset - init.size,
        )).asInstanceOf
...
case class AdjacentVect[N <: Int, M <: Int, H, I <: Vect[N, H], T <: Vect[M, H]](init: I, tail: T)
    extends Vect[N + M, H]:
