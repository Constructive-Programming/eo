...
object ForgetfulFunctor:
  ...
  /** Direct `Either` instance — unblocks `.modify` / `.replace`
    * on every Prism (`Either`-carrier) optic.
    *
    * @group Instances */
  ...
  /** Fallback derivation for any carrier with a `Bifunctor[F]` —
    * maps only the right parameter via `bimap(identity, f)`.
    *
    * @group Instances */
  ...
  /** Fallback derivation for any profunctor carrier — uses the
    * right-side mapping (`rmap`) because ForgetfulFunctor only
    * touches the focus parameter.
    *
    * @group Instances */
