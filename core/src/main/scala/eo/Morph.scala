...
/** Picks the direction in which to morph a pair of optics into a shared carrier, so that
  * `.andThen` can be called on optics whose carriers differ.
  *
  ...
  *   - [[Morph.leftToRight]] — when a `Composer[F, G]` exists, morph the left (outer) side
  *     into `G`; `Out = G`.
  *   - [[Morph.rightToLeft]] — when a `Composer[G, F]` exists, morph the right (inner) side
  *     into `F`; `Out = F`.
  ...
  * `bothViaAffine` instance lives in a lower-priority parent trait so it only kicks in when
  * the higher-priority ones are all inapplicable.
  *
  * @tparam F outer optic's carrier
  * @tparam G inner optic's carrier
  */
...
/** Low-priority `Morph` instances — consulted only when the higher-priority instances in
  * [[Morph$]] don't apply. Kept in a separate trait purely for given-priority purposes.
  */
...
object Morph extends LowPriorityMorphInstances:
  ...
  /** Same carrier — no morphing. Most-specific given, wins overload resolution whenever
    * `F = G`.
    *
    * @group Instances
    */
