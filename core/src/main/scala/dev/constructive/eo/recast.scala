...
/** Zero-alloc phantom recasts for `Either`, the analogue of `data.Affine.Miss.widenB`: a `Left`
  ...
  * phantom side is a cast, not a rebuild. Composition kernels and carrier bridges use these to
  * pass miss / hit wrappers through unchanged instead of re-allocating `Left(t)` / `Right(c)` at
  * every hop. (The stdlib's `withRight` / `withLeft` are upcasts only — they can't re-type the
  * phantom side to an unrelated type.)
  */
