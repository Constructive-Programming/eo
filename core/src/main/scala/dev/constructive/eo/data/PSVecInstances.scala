...
/** cats typeclass instances for [[PSVec]] — required so `MultiFocus[PSVec]` can absorb the
  ...
  * identical in shape to the legacy `PowerSeries.traverse`.
  *
  * Lives in a separate file to keep `PSVec.scala` free of cats imports — the carrier itself is
  * data-only. Instances are top-level `given` at package `data` scope so they are visible to every
  * `MultiFocus[PSVec]` `mfFunctor` / `mfFold` / `mfTraverse` derivation site without explicit
  * imports.
  */
