...
/** Direct coverage for internal machinery that shipped without user-facing specs.
  ...
  *   3. [[dev.constructive.eo.Reflector]] instance surface — `ap`, `pure`, `map`, `product`
  *      delegation overrides for `List` / `ZipList` / `Const[M, *]`. Kaleidoscope specs exercise
  *      `reflect` but not the cats `Apply` overrides.
  */
