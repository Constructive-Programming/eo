...
class OpticsBehaviorSpec extends Specification with ScalaCheck:
  ...
  // ----- ForgetfulFold surface: foldMap / headOption / length / exists -----
  //
  // ONE property, TWO receiver shapes, one independent oracle (the expected focus
  // List supplied per row). `extAgrees` holds the optic as the BARE `Optic[…]` — the
  // only way to reach the Optic.scala EXTENSION, since `Traversal` and `ForgetFold`
  // (via `CanFold`) both declare member twins that win by precedence. `cfAgrees`
  // holds the same fold as a `CanFold`, reaching the trait's default bodies.
  // No higher rung is reachable: `laws/` ships no fold-surface ruleset to register
  // against, and promoting `exists(p) === foci.exists(p)` to a law is user-gated.
  //
  // covers: optics/Optic.scala:443 — the `||` of the inline disjunction monoid. Needs
  //   a MULTI-FOCUS carrier (Tuple2's foldMap is `f(a)`, Affine/Either's is
  //   `fold(_ => empty, f)`; neither ever evaluates `combine`) AND an input where some
  //   focus satisfies `p` — k in {-1, 4} with n >= 1.
  // covers: CanFold.scala:27 — the `false` identity of the same monoid. Needs the
  //   no-focus-satisfies class: k = 10 (elements are 0..9), or the empty fold n = 0.
  // Carrier rows carried over from the example block this replaces: Tuple2 (Lens, one
  //   focus), Either (Prism, hit and miss), MultiFocus[PSVec], Forget[List].
  "ForgetfulFold surface: foldMap / headOption / length / exists agree with the foci" >> {
    def extAgrees[S, T, A, B, F[_, _]](o: Optic[S, T, A, B, F])(
        s: S,
        expected: List[A],
        p: A => Boolean,
    )(using ForgetfulFold[F]): Boolean =
      val foci: List[A] = o.foldMap[List[A]](a => List(a))(s)
      foci == expected && o.headOption(s) == expected.headOption &&
      o.length(s) == expected.length && o.exists(p)(s) == expected.exists(p)

    def cfAgrees[S, A](cf: CanFold[S, A])(s: S, expected: List[A], p: A => Boolean): Boolean =
      cf.foci(s) == expected && cf.headOption(s) == expected.headOption &&
        cf.length(s) == expected.length && cf.exists(p)(s) == expected.exists(p)
    ...
    val eachOptic: Optic[List[Int], List[Int], Int, Int, MultiFocus[PSVec]] =
      Traversal.each[List, Int]
    val foldOptic: Optic[List[Int], Unit, Int, Unit, Forget[List]] = Fold[List, Int]
    val listCanFold: CanFold[List[Int], Int] = Fold[List, Int]

    val rows: Gen[(List[Int], Int)] = for
      n <- Gen.oneOf(0, 1, 2, 5, 17)
      xs <- Gen.listOfN(n, Gen.choose(0, 9))
      k <- Gen.oneOf(-1, 4, 10)
    yield (xs, k)
    ...
    Prop.forAll(rows) {
      case (xs, k) =>
        val p = (n: Int) => n > k
        extAgrees(ageL)(("Alice", 30), List(30), p) &&
        extAgrees(posIntPrism)(5, List(5), p) &&
        extAgrees(posIntPrism)(-1, List.empty[Int], p) &&
        extAgrees(eachOptic)(xs, xs, p) &&
        extAgrees(foldOptic)(xs, xs, p) &&
        cfAgrees(listCanFold)(xs, xs, p)
    }
  }
