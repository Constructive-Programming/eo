...
/** The metamorphism — fold→unfold, the direction-dual of [[FusionSpec]]'s hylo — and the proof that
  ...
  * cannot deforest: there is no shared functor whose `project ∘ embed` could cancel, so the neck `A`
  * is materialised (the `Meta` existential is `X = A`).
  *
...
class MetaSpec extends Specification:
  ...
  private val spine: Int => RoseF[Int] = n => if n <= 0 then RoseF(0, Nil) else RoseF(n, List(n - 1))
  ...
  "metaChrono folds course-of-value (histo) then multi-layer-unfolds (futu)" >> {
    ...
    val futuSpine: Int => RoseF[Coattr[RoseF, Int]] = n =>
      if n <= 0 then RoseF(0, Nil) else RoseF(n, List(Coattr.Pure(n - 1)))
  ...
  "histo.meta(futu) seam == Schemes.metaChrono" >> {
    ...
    val futuSpine: Int => RoseF[Coattr[RoseF, Int]] = n =>
      if n <= 0 then RoseF(0, Nil) else RoseF(n, List(Coattr.Pure(n - 1)))
    val viaSeam = Schemes.histo[BinF, Bin, Int](histoSum).meta(Schemes.futu[RoseF, Int, Rose](futuSpine))
