...
/** Histomorphism citizen — a course-of-value fold worn as an optic over [[Scheme]] with **`X =
  ...
  * `Histo : Cata :: Lens : Getter` — the index refined from the trivial comonad up to the cofree
  * one.
  *
  ...
  * (`Optic[S, Unit, A, Unit, Scheme]`), consumed via `.get`; the read projects the root's head
  * ([[Attr.forget]]).
  *
...
final class Histo[F[_], S, A](private[zoo] val alg: F[Attr[F, A]] => A)(using
  ...
  /** Metamorphism at the universal indices — the **fold→unfold dual of [[Futu.cross]]**'s chrono
    ...
    */
  def meta[G[_], T](futu: Futu[G, A, T]): Meta[S, A, T] =
    new Meta[S, A, T](run.andThen(futu.build))
