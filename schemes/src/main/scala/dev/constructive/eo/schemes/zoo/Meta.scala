...
/** Metamorphism citizen — a **fold-then-unfold** worn as an optic over [[Scheme]], reading `S => T`
  ...
  * consumes.
  *
  * That non-trivial `X` is the thesis stating the obvious honestly: `meta` **cannot fuse**. It folds
  * a functor `F` down to `A`, then unfolds a *different* functor `G` back up; with `F ≠ G` there is
  * no `project ∘ embed` cancellation to ride, so `A` is genuinely materialised. (Contrast `hylo` /
  * `chrono`, whose single shared functor makes the neck cancel — `X = Nothing`.) Built by
