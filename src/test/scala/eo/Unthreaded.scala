...
@main def unthreaded =
  ...
  val personPhone =
    Lens[Person, ArraySeq[Phone]](_.phones, (s, b) => s.copy(phones = b))
      ...
      .andThenLeft[Phone, Phone](Traversal.each[ArraySeq, Phone])
  ...
  val personPipeline: Json => Json = ???
  val phonePipeline: Json => Json = ???

  val personPhonesAreMobile: Optic[Person, Person, Boolean, Boolean, Forget[ArraySeq]] = ???
  val personPlaceIsMobile: Optic[Person, Json, Boolean, Boolean, Forget[ArraySeq]] = ???
