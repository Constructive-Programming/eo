...
/** Law equations for an `Optional[S, A]` — `Optic[S, S, A, A, Affine]`.
  *
  * Ported from Monocle's `monocle.law.OptionalLaws`. `getOption` is
  * recovered from EO's primitive `to`, whose `Affine` carrier is
  * essentially `Either[S, (X, A)]` — the `.affine.toOption.map(_._2)`
  * chain projects out the focused value when present.
  */
