...
/** Carrier-level laws for `Affine[X, A]`.
  ...
  * the optic level (see [[eo.laws.eo.OptionalComposeLaws]]); re-stating its associativity equations
  * as a standalone law class would duplicate that coverage without adding signal.
  */
trait AffineLaws[X, A]:
  ...
  /** A sample `Affine` value produced by the concrete subclass. The subclass typically supplies
    ...
    * [[eo.laws.data.discipline.AffineTests]].
    */
