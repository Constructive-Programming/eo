...
/** Laws that pin down `Optic.morph` — EO's carrier-coercion extension.
  *
  * A1/I1. Morph preserves `modify`: reshaping the carrier does not change what the optic does to
  * the focus.
  *
  * A2. Morph preserves `get`: for any carriers `F` / `G` that both expose `Accessor`, reshaping
  * preserves the read path as well.
  *
  * These two laws are the foundation of EO's "write once, reuse everywhere" claim about optics. If
  * they failed, a user could not safely convert a `Lens` to an `Optional` or a `Getter` to a `Fold`
