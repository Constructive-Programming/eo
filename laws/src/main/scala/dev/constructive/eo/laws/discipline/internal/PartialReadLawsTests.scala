...
/** Internal parent `RuleSet` for the partial-read law families — `Optional` and `Prism`. Both pin
  * down `modifyIdentity`, `composeModify`, and `consistentGetOptionModifyId` (the partial-read
