...
/** Discipline `RuleSet` for [[PrismLaws]].
  *
  * '''Hierarchy:''' inherits the three partial-read props from `internal.PartialReadLawsTests` and
  * adds the two reverseGet round-trip props (`partial round trip one way` and
