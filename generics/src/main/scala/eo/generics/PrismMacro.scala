...
/** Compile-time derivation of a `Prism` from a parent sum / enum /
  * union type `S` to one of its direct child types `A <: S`.
  *
  * Usage:
  * {{{
  ...
  *     [[hearth.typed.Classes.Enum]] view: `Enum.parse[S]` recognises
  *     sealed traits, Scala 3 enums, AND union types; `matchOn`
  *     generates an exhaustive pattern match over `directChildren`,
  *     using `MatchCase.eqValue` for parameterless enum cases /
  *     Java enum vals (which would otherwise lose their type at
  *     erasure) and `MatchCase.typeMatch` for the rest.
  *   - The reconstruct is a plain upcast `(a: A) => a: S`, valid for
  *     every well-formed `prism[S, A <: S]`.
  *   - Validation: we check at macro time that `A` actually appears
  *     among `directChildren` of `S`, and produce a useful error if
  *     not (e.g. when the user writes `prism[Shape, OtherEnum.Foo]`).
  */
object PrismMacro:
...
/** Hearth-backed Prism macro implementation.
  */
private final class HearthPrismMacro(q: Quotes)
